function main() {
  ...
  function handleGetImcMsg(imc) {
    if(imc < 18.5) {
      return `${nome} Seu IMC é ${imc} (Abaixo do peso).`;
    }
    
    if(imc <= 24.9) {
      return `${nome} Seu IMC é ${imc} (Peso normal).`;
    }
    
    if(imc <= 29.9) {
      return `${nome} Seu IMC é ${imc} (Sobrepeso).`;
    }
    
    if(imc <= 34.9) {
      return `${nome}Seu IMC é ${imc} (Obesidade grau 1).`;
    }
    
    if(imc <= 39.9) {
      return `${nome} Seu IMC é ${imc} (Obesidade grau 2).`;
    }
    
    if(imc >= 40) {
      return `${nome} Seu IMC é ${imc} (Obesidade grau 3).`;
    }
  }
